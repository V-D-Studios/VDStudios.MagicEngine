...
public static class GraphicsDeviceExtensions
{
    ...
    {
        device.UpdateBuffer(buffer, bufferOffsetInBytes, ref data[0], (uint)(Unsafe.SizeOf<T>() * data.Length));
    }
    ...
    /// <param name="data">A <see cref="Span{T}"/> containing the data to upload</param>
    /// <param name="bufferOffsetInBytes">An offset, in bytes, from the beginning of <paramref name="buffer"/>'s storage, at which new data will be uploaded</param>
    public static void UpdateBuffer<T>(this CommandList commandList, DeviceBuffer buffer, uint bufferOffsetInBytes, Span<T> data) where T : unmanaged
    {
        commandList.UpdateBuffer(buffer, bufferOffsetInBytes, ref data[0], (uint)(Unsafe.SizeOf<T>() * data.Length));
    }
}
